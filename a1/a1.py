...
def bfs(graph, root, max_depth):
    ...
    while q:
        v=q.popleft()        
        path=node2num_paths[v]
        current_distance=node2distances[v]

        if((current_distance >= max_depth)or(current_distance<0)):
            break
        ...
        for neighbor in graph[v]:
            ...
            if node2distances[neighbor]==current_distance+1:
                try:
                    node2parents[neighbor].append(v)
                except:
                    node2parents[neighbor] = [v]
                try:
...
def bottom_up(root, node2distances, node2num_paths, node2parents):
    ...
    ###TODO
    bottomUpResult = {}
    sortedNode2Distances = sorted(node2distances.items(), key=lambda x:-x[1])
    val=0
    resultDict={key:val for key in node2distances}
    
    for eachTuple in sortedNode2Distances:
        node = eachTuple[val]
        if node != root:
            temp=(1.0 + resultDict[node] )
            score = (temp/ node2num_paths[node])
            if (score<math.inf):
                for parentNode in node2parents[node]:
                    tup = (node, parentNode)
                    edge = sorted(tup)
                    bottomUpResult[(edge[val],edge[1])] = score
                    resultDict[parentNode] += score
            else:
                print("wrong score")
...
def approximate_betweenness(graph, max_depth):
    ...
    """
    ###TODO
    returnResultOfBottomUp = defaultdict(int)
    storeResult = defaultdict(int)

       
    for node in graph.nodes():
        node2distances, node2num_paths, node2parents=bfs(graph,node,max_depth)
        returnResultOfBottomUp = bottom_up(node,node2distances, node2num_paths, node2parents)
        for key,value in returnResultOfBottomUp.items():
            storeResult[tuple(key)] = (storeResult[key] + returnResultOfBottomUp[key])
            
   
    """for key,value in ans:
        #print("dict coming key::",key)
        print("dict coming value::",value)"""

    for key,value in storeResult.items():
        val = value/2
        storeResult[key]= val
...
def partition_girvan_newman(graph, max_depth):
    ...
    between=sorted(temp.items(),key=lambda x:-x[1])
    graphs=list()
    i=0
    while len(graphs)<=1:
...
def get_subgraph(graph, min_degree):
    ...
    """
    ###TODO
    edgeOutDegree=graph.degree()
    toKeep=[n for n in edgeOutDegree if edgeOutDegree[n]>=min_degree]
    return graph.subgraph(toKeep)
    pass
...
def volume(nodes, graph):
    ...
    """
    ###TODO
    lengthEdges=len(graph.edges(nodes))
    return lengthEdges
    pass
...
def norm_cut(S, T, graph):
    ...
    volS=float(volume(S,graph))
    volT=float(volume(T,graph))
    normCutValue=(float(cutSet/volS)+float(cutSet/volT))
...
def score_max_depths(graph, max_depths):
    ...
    for depth in max_depths:
        cluster = partition_girvan_newman(graph,depth)
        tup.append((depth,norm_cut(cluster[0].nodes(),cluster[1].nodes(),graph)))
...
def make_training_graph(graph, test_node, n):
    ...
    edgesToRemove=train_graph.neighbors(test_node)
    
    edgesToRemove.sort()
            
    """i=0
    for v in edgesToRemove[:n]:
        if i<n:
            train_graph.remove_edge(test_node,v)
        else:
            break;
        i=i+1"""
    for n in edgesToRemove[:n]:
        train_graph.remove_edge(test_node,n)
...
def jaccard(graph, node, k):
    ...
    for n in graph.nodes():
        if n != node and not graph.has_edge(node, n):
            ...
            lengthNebrIntersection=len(neighborsi & neighborsj)
            lengthNebrUnion=len(neighborsi | neighborsj)
            resultantScore = 1. * (lengthNebrIntersection) / (lengthNebrUnion)
            scores.append(((node, n), resultantScore))

    greaterThanZeroScores=[]
    equalsToZeroScores=[]
    for item in scores:
        if (item[1]>0.0):
            greaterThanZeroScores.append(item)
        elif (item[1]==0.0):
            equalsToZeroScores.append(item)
    scores=(sorted(greaterThanZeroScores, key=lambda x: (x[1]),reverse=True))
    scores+=(sorted(equalsToZeroScores, key=lambda x: (x[0]),reverse=False))   
...
def path_score(graph, root, k, beta):
    ...
    node2distances, node2num_paths, node2parents = bfs(graph,root,test)

    for node in graph.nodes():
        if node != root and not graph.has_edge(root, node):
            scores.append(((root, node), (beta**node2distances[node])*node2num_paths[node]))
            
    scores = sorted(scores, key=lambda x: (-x[1],x[0]))[:k]
    return scores
    pass
...
def evaluate(predicted_edges, graph):
    ...
    """
    ###TODO
    return 1.*len([edge for edge in predicted_edges if graph.has_edge(*edge)])/len(predicted_edges)
    pass
